...
const middleTruncate = (/** @type {string} */ str) => (str.length > 50 ? (`${str.slice(0, 25)}…${str.slice(-25)}`) : str);
...
module.exports = {
   ...
   * @returns {[allowedValues: string[] | RegExp, using: string]}
   */
  readAllowedValues: (context) => {
    /** @type {{ allowedValues?: string[], markdownFile?: string, packageJson?: string, pattern?: string, prependAtSign: boolean }} */
    ...
    if (option) {
      ...
      if (option.allowedValues) {
        ...
        return [new RegExp(option.pattern), `pattern '${middleTruncate(option.pattern)}'`];
      } else {
        throw new Error(
